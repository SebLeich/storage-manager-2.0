...
import { IBpmnJS, IFunction, IInputParam, IMethodEvaluationResult, IParam, IParamDefinition, IProcessBuilderConfig, PROCESS_BUILDER_CONFIG_TOKEN } from "@/lib/process-builder/interfaces";
...
import { firstValueFrom, map, shareReplay, timer } from "rxjs";
import { selectIInterface, selectIParam, selectIParams, selectNextParameterIdentifier } from "@/lib/process-builder/store/selectors";
...
export class DataC2MProcessor implements IC2mProcessor {
    ...
    public async processConfiguration({ taskCreationPayload, taskCreationFormGroupValue }: { taskCreationPayload: ITaskCreationPayload, taskCreationFormGroupValue?: ITaskCreationFormGroupValue }, c2mProcessingObjects: Partial<C2mProcessingObjects>) {
        ...
        if (!configureActivity || !updatedFunction || !taskCreationFormGroupValue) {
            return;
        }
