...
import {
  defaultGoodEdgeColor,
  generateGuid,
  infinityReplacement,
  keyboardControlMoveStep,
  selectedGoodEdgeColor,
...
export class VisualizerComponentService {
  ...
  private _container: ReplaySubject<Container> = new ReplaySubject<Container>(
    1
  );
  private _visualizerWrapper: ReplaySubject<ElementRef<HTMLDivElement>> =
    new ReplaySubject<ElementRef<HTMLDivElement>>(1);
  private _hoverIntersections: ReplaySubject<MouseEvent> =
    new ReplaySubject<MouseEvent>(1);
  private _hoveredElement: BehaviorSubject<{
    preview: string;
    groupColor: string;
    groupId: number;
    seqNr: number;
  ...
  private _selectedElement: BehaviorSubject<{
    preview: string;
    groupColor: string;
    groupId: number;
    seqNr: number;
  ...
  hoveredGood$ = combineLatest([this.container$, this.hoveredElement$]).pipe(
    map(([container, element]) => {
      return element
        ? container.goods.find((x) => x.id === element.goodId)
        : null;
    })
  );
  selectedElement$ = this._selectedElement.asObservable();
  selectedGood$ = combineLatest([this.container$, this.selectedElement$]).pipe(
    map(([container, element]) => {
      return element
        ? container.goods.find((x) => x.id === element.goodId)
        : null;
  ...
  addContainerToScene(container: Container, groups: Group[]) {
    ...
    this.addBaseGridToScene(container.height, container.length);
    this.addBoxToScene(DataService.getContainerDimension(container));
    this._container.next(container);
    this._addElementToScene(
      DataService.getContainerDimension(container),
      'Container',
      'bordered',
      null,
      null,
    ...
    for (var good of container.goods) {
      ...
      this._addElementToScene(
        DataService.getGoodDimension(good),
        `${good.desc}`,
        'good',
        good.sequenceNr,
        good.id,
  ...
  animateStep(
    step: Step,
    keepPreviousGoods: boolean = false,
    keepPreviousUnusedSpaces: boolean = false
  ) {
    this._dataService.currentContainer$
      .pipe(take(1))
      .subscribe((container: Container) => {
        this.clearScene(
          keepPreviousGoods,
          keepPreviousUnusedSpaces,
          [step.usedDimension?.guid ?? null].filter((x) => x != null)
        );
        this._addUnloadingArrowToScene(container.height, container.length);
        this.addBaseGridToScene(container.height, container.length);
        this.addBoxToScene(DataService.getContainerDimension(container));
        ...
        for (let unusedDimension of step.unusedDimensions) {
          this._addElementToScene(
            unusedDimension,
            null,
            unusedDimension.length === Infinity
              ? 'infiniteSpace'
              : 'unusedSpace',
            step.sequenceNumber,
            null,
  ...
  clearScene(
    keepPreviousGoods: boolean = false,
    keepPreviousUnusedSpaces: boolean = false,
    removeDimensionsAnyway: string[] = []
  ) {
    let remove = this.scene.children.filter(
      (child) =>
        removeDimensionsAnyway.indexOf(child.userData.dimensionGuid) > -1 ||
        !(
          (keepPreviousGoods && child.userData.type === 'good') ||
          (keepPreviousUnusedSpaces &&
            (child.userData.type === 'infiniteSpace' ||
              child.userData.type === 'unusedSpace'))
        )
    );
  ...
  highlightGood(good: Good) {
    let meshes = this.scene.children.filter(
      (x) => x instanceof ThreeJS.Mesh
    ) as ThreeJS.Mesh[];
    meshes.forEach((x) => {
      let meshWrapper = this._meshes.find((y) => y.mesh === x);
      ((x as ThreeJS.Mesh).material as ThreeJS.MeshBasicMaterial).color.set(
        meshWrapper
          ? meshWrapper.goodId === good.id
            ? 'white'
            : meshWrapper.groupColor
          : null
      );
    });
  }
  ...
  highlightGoods(goods: Good[]) {
    ...
    meshes.forEach((x) => {
      ...
      ((x as ThreeJS.Mesh).material as ThreeJS.MeshBasicMaterial).color.set(
        meshWrapper
          ? goodIds.indexOf(meshWrapper.goodId) > -1
            ? 'white'
            : meshWrapper.groupColor
          : null
      );
    ...
    switch (event.key) {
      case 'w':
        this.controls.target.set(
          ...
          this.controls.target.y - keyboardControlMoveStep,
      ...
      case 'a':
        this.controls.target.set(
          this.controls.target.x + keyboardControlMoveStep,
      ...
      case 'y':
        this.controls.target.set(
          this.controls.target.x,
          this.controls.target.y,
          this.controls.target.z + keyboardControlMoveStep
        );
        updateControls = true;
        break;
      case 'x':
  ...
  mouseclick(event: MouseEvent) {
    if ((event.target as HTMLCanvasElement).id === this._sceneBodyId) {
      this.visualizerWrapper$.pipe(take(1)).subscribe((wrapper) => {
        ...
        this._selectedElement.next(
          pointedElement
            ? this._meshes.find((x) => x.mesh === pointedElement.object)
            : null
        );
      });
    }
  ...
  reRenderCurrentContainer() {
    combineLatest([
      this._dataService.currentContainer$,
      this._dataService.currentGroups$,
    ])
      .pipe(take(1))
      .subscribe(([container, groups]: [Container, Group[]]) =>
  ...
  setSceneDimensions(
    width: number,
    height: number,
    preventCameraPositionReset: boolean = false
  ) {
    ...
    this._dataService.currentGroups$
      ...
      .subscribe((groups: Group[]) => {
        this._meshes.forEach((x) => {
          ...
          (
            (x.mesh as ThreeJS.Mesh).material as ThreeJS.MeshBasicMaterial
          ).color.set(x.groupColor);
        });
      });
  }

  public addBaseGridToScene(
    ...
    gridHelper.position.set(originX, originY === null? containerHeight / -2: originY, originZ);
    this.scene.add(gridHelper);
  }

  public addBoxToScene(dimension: Dimension) {
    this._addElementToScene(
      dimension,
      'Container',
      'bordered',
  ...
  private _addElementToScene(
    dimension: Dimension,
    preview: string,
    type: null | 'bordered' | 'good' | 'infiniteSpace' | 'unusedSpace',
    sequenceNumber: number = null,
    goodId: number = null,
    group: Group = null,
    parentDimension: Dimension = null,
    dimensionGuid: string = null
  ) {
    ...
    switch (type) {
      case 'good':
        ...
        var geometry = new ThreeJS.BoxGeometry(
          dimension.width,
          dimension.height,
          length,
          4,
      ...
      default:
        let edgeColor =
          type === 'infiniteSpace'
            ...
            ? 'red'
            : 'black';
        var geometry = new ThreeJS.BoxBufferGeometry(
          dimension.width,
          dimension.height,
          dimension.length === Infinity ? infinityReplacement : dimension.length
        );
        edges = new ThreeJS.LineSegments(
          new ThreeJS.EdgesGeometry(geometry),
          new ThreeJS.LineBasicMaterial({ color: edgeColor, linewidth: 1 })
        );
        edges.userData = { type: type, dimensionGuid: dimensionGuid };
    ...
    var x = parentDimension
      ? dimension.x - parentDimension.width / 2 + dimension.width / 2
      : dimension.x;
    var y = parentDimension
      ? dimension.y - parentDimension.height / 2 + dimension.height / 2
      : dimension.y;
    var z = parentDimension
      ? dimension.z -
        parentDimension.length / 2 +
        (dimension.length === Infinity
          ? infinityReplacement
          : dimension.length) /
          2
      : dimension.z;
  ...
  private _addUnloadingArrowToScene(
    containerHeight: number,
    containerLength: number
  ) {
    var from = new ThreeJS.Vector3(
      0,
      containerHeight / -2,
      containerLength / 2
    );
    var to = new ThreeJS.Vector3(
      0,
      containerHeight / -2,
    ...
    var arrowHelper = new ThreeJS.ArrowHelper(
      direction.normalize(),
      from,
      length,
      'red'
  ...
  private _getPointedElement(
    event: MouseEvent,
    wrapper: ElementRef<HTMLDivElement>,
    meshes: ThreeJS.Mesh[] = null
  ) {
    ...
    let x =
      ((event.clientX -
        (wrapper.nativeElement.offsetParent as HTMLDivElement).offsetLeft -
        (wrapper.nativeElement as HTMLDivElement).offsetLeft) /
        (event.target as HTMLCanvasElement).offsetWidth) *
        2 -
      1;
    let y =
      -(
        (event.clientY - (event.target as HTMLCanvasElement).offsetTop) /
        (event.target as HTMLCanvasElement).offsetHeight
      ) *
        2 +
      1;
  ...
  private _highlightIntersections(
    event: MouseEvent,
    wrapper: ElementRef<HTMLDivElement>
  ) {
    let meshes = this.scene.children.filter(
    ...
    meshes.forEach((x) => {
      let meshWrapper = this._meshes.find((y) => y.mesh === x);
      ((x as ThreeJS.Mesh).material as ThreeJS.MeshBasicMaterial).color.set(
        meshWrapper ? meshWrapper.groupColor : null
      );
    });
    ...
    this._hoveredElement.next(
      found && found.object
        ? this._meshes.find((x) => x.mesh === found.object)
        : null
    );
  }
      ...
      ...[
        ...
        this.selectedElement$.subscribe((element) => {
          for (let mesh of this._meshes) {
            if (mesh && mesh.edges) {
              let color =
                mesh === element ? selectedGoodEdgeColor : defaultGoodEdgeColor;
              (mesh.edges.material as ThreeJS.LineBasicMaterial).color =
                new ThreeJS.Color(color);
            }
          }
        }),
